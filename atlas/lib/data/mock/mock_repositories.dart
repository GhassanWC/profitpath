...
final class MockUserRepository implements UserRepository {
  ...
  Future<AtlasUser> byId(String id) async {
    await _backend.wait(0.4);
    final AtlasUser? user = _backend.users[id];
    if (user == null) {
  ...
  @override
  Future<AtlasUser?> byUsername(String username) async {
    await _backend.wait(0.4);
    final String needle = username.toLowerCase().replaceFirst('@', '');
    for (final AtlasUser user in _backend.users.values) {
  ...
  Future<void> setFollowing({
    ...
    final AtlasUser? target = _backend.users[targetId];
    if (target != null) {
  ...
  @override
  Future<AtlasUser> update(AtlasUser user) async {
    await _backend.wait();
