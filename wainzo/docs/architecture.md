...
Wainzo exists to answer one question:
...
- **The data layer throws `WainzoException` and nothing else.** Transport and
...
**No credentials on the device.** `RemoteAiService` calls the Wainzo backend,
...
4. Point `WAINZO_API_BASE_URL` at the backend that verifies the identity token
