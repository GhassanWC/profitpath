...
class SettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final WainzoUser? user = ref.watch(currentUserProvider);
    final AppConfig config = ref.watch(appConfigProvider);

    if (user == null) {
      return const Scaffold(body: WainzoLoading());
    }
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text(
                          ...
                          'will be reached in. Wainzo never asks for your location.',
            ...
            child: Text(
              config.isMock
                  ? 'Running on the bundled sample content. No network calls are made, '
                        ...
                        'through the Wainzo backend when one is configured.'
    ...
    return WainzoCard(
      child: switch (profile) {
        ...
        AsyncError<InterestProfile>(error: final Object error) =>
          WainzoErrorView(error: error, compact: true),
        _ => const WainzoLoading(compact: true),
      },
    );
  }
}
