...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final AsyncValue<WainzoUser> user =
        isMine && ref.watch(currentUserProvider) != null
        ? AsyncValue<WainzoUser>.data(ref.watch(currentUserProvider)!)
        : ref.watch(userByIdProvider(userId));

    return Scaffold(
      body: switch (user) {
        AsyncData<WainzoUser>(value: final WainzoUser data) => _Body(
          user: data,
          isMine: isMine,
          tab: _tab,
          onTab: (int value) => setState(() => _tab = value),
        ),
        AsyncError<WainzoUser>(error: final Object error) => SafeArea(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return CustomScrollView(
      slivers: <Widget>[
        SliverAppBar(
          pinned: true,
          backgroundColor: WainzoColors.ground,
          title: Text(user.handle, style: WainzoTypography.title),
          actions: <Widget>[
            if (isMine)
              IconButton(
        ...
        switch (posts) {
          ...
          AsyncError<List<Post>>(error: final Object error) =>
            SliverToBoxAdapter(
              child: WainzoErrorView(error: error, compact: true),
            ),
          _ => const SliverToBoxAdapter(child: WainzoLoading(compact: true)),
        },
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          if (user.bio.isNotEmpty) ...<Widget>[
            ...
            Text(user.bio, style: WainzoTypography.bodyMuted),
          ],
          const SizedBox(height: Insets.lg),
          Row(
            children: <Widget>[
            ...
            Wrap(
              ...
              children: <Widget>[
                for (final String interest in user.interests)
                  WainzoChip(label: interest, dense: true),
              ],
            ),
        ],
      ),
    ...
    child: Column(
      ...
      children: <Widget>[
        Text(
          ...
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
          style: WainzoTypography.headlineSmall.copyWith(fontSize: 20),
        ),
        Text(
          label.toUpperCase(),
          maxLines: 1,
