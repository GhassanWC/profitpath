...
class WorldMapPainter extends CustomPainter {
  ...
  void _paintOcean(Canvas canvas, Size size) {
    canvas.drawRect(Offset.zero & size, Paint()..color = WainzoColors.ocean);
  ...
  void _paintLand(Canvas canvas, Size size, double scale, double hairline) {
    ...
    for (final CountryOutline outline in outlines.all) {
      ...
      fill.color = isHighlighted
          ? Color.lerp(WainzoColors.land, WainzoColors.accent, 0.55)!
    ...
    if (selectedPath != null) {
      ...
      canvas.drawPath(
        ...
        Paint()
          ...
          ..color = WainzoColors.accent.withValues(
            alpha: 0.10 + 0.10 * (1 - pulse),
          )
          ..maskFilter = MaskFilter.blur(BlurStyle.normal, hairline * 8),
      );
      canvas.drawPath(selectedPath, Paint()..color = WainzoColors.accent);
      canvas.drawPath(
        selectedPath,
        Paint()
          ..style = PaintingStyle.stroke
  ...
  Color _activityColour(int count) {
    if (count <= 0 || maxActivity <= 0) return WainzoColors.land;
    final double t = math.log(count + 1) / math.log(maxActivity + 1);
    return Color.lerp(
      WainzoColors.land,
      WainzoColors.landActive,
      t.clamp(0.0, 1.0),
    )!;
  }

  void _paintMarkers(Canvas canvas, Size size, double scale) {
    final Paint dot = Paint()..color = WainzoColors.accent;
    final Paint halo = Paint()
      ..color = WainzoColors.accent.withValues(alpha: 0.16)
  ...
  TextPainter _label(String text, bool emphasised) {
    ...
    final TextPainter painter = TextPainter(
      text: TextSpan(
        ...
        style:
            (emphasised
                    ? WainzoTypography.titleSmall
                    : WainzoTypography.overline)
                .copyWith(
                  color: emphasised ? WainzoColors.ink : WainzoColors.inkMuted,
                  shadows: const <Shadow>[
                    Shadow(color: WainzoColors.ocean, blurRadius: 6),
                    Shadow(color: WainzoColors.ocean, blurRadius: 2),
                  ],
                ),
      ),
