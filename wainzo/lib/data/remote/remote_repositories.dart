...
///
/// These are the implementations the app runs on once `WAINZO_BACKEND=remote`
/// and `WAINZO_API_BASE_URL` are set. They are deliberately thin: the server
...
final class RemoteUserRepository implements UserRepository {
  ...
  Future<WainzoUser> byId(String id) async =>
      WainzoUser.fromJson(await _api.getJson('/v1/users/$id'));

  @override
  Future<WainzoUser?> byUsername(String username) async => WainzoUser.fromJson(
    await _api.getJson(
  ...
  @override
  Future<WainzoUser> update(WainzoUser user) async => WainzoUser.fromJson(
