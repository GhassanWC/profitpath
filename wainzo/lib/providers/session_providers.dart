...
final class AuthController extends AsyncNotifier<AuthSession?> {
  ...
  Future<void> deleteAccount() async {
    final WainzoUser? user = state.value?.user;
    if (user != null) {
  ...
  Future<void> updateProfile(WainzoUser updated) async {
...
final Provider<WainzoUser?> currentUserProvider = Provider<WainzoUser?>(
  (Ref ref) => ref.watch(authControllerProvider).value?.user,
);

/// Where the viewer is, as a country code.
///
/// It comes from their profile, never from GPS. Wainzo decides what reaches
